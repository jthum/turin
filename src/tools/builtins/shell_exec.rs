...
impl Tool for ShellExecTool {
    ...
    async fn execute(
        ...
        let timeout_seconds = args.timeout_seconds.min(MAX_TIMEOUT_SECONDS);
        ...
        let read_stream = async {
            ...
            loop {
                tokio::select! {
                    res = stdout.read(&mut buf_stdout), if !stdout_done => {
                        match res {
                            Ok(0) => stdout_done = true,
                            Ok(n) => {
                                if stdout_buf.len() < MAX_OUTPUT_BYTES {
                                    let take = n.min(MAX_OUTPUT_BYTES - stdout_buf.len());
                                    stdout_buf.extend_from_slice(&buf_stdout[..take]);
                                }
                            }
                            Err(_) => stdout_done = true,
                        }
                    }
                    res = stderr.read(&mut buf_stderr), if !stderr_done => {
                        match res {
                            Ok(0) => stderr_done = true,
                            Ok(n) => {
                                if stderr_buf.len() < MAX_OUTPUT_BYTES {
                                    let take = n.min(MAX_OUTPUT_BYTES - stderr_buf.len());
                                    stderr_buf.extend_from_slice(&buf_stderr[..take]);
                                }
                            }
