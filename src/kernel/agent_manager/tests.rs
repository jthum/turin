...
async fn build_shared_peer_kernel_reuses_configured_tool_registry() -> anyhow::Result<()> {
    ...
    let peer_kernel = super::peer_session::fork_peer_kernel(&kernel.host.agent_manager);

    assert_eq!(peer_kernel.tool_registry.len(), registry.len());
    assert!(peer_kernel.tool_registry.get("test_tool").is_some());
    assert!(Arc::ptr_eq(
        &kernel.host.persistence_locks,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn runtime_signals_dispatch_through_an_injected_adapter_and_reload_topics()
-> anyhow::Result<()> {
    let tmp = tempdir()?;
    ...
    runtime_store
        .insert_signal(crate::persistence::state::SignalInsert {
            ...
            source_agent_id: "default".to_string(),
        ...
        .await?;
    kernel.agent_manager().wake_agent("reviewer").await?;

    let deadline = tokio::time::Instant::now() + std::time::Duration::from_secs(5);
    loop {
        if runtime_store
            .list_signals_for_agent("reviewer", None, 10)
            .await?
            .is_empty()
        {
            break;
        ...
        if tokio::time::Instant::now() >= deadline {
            anyhow::bail!("timed out waiting for adapter signal dispatch");
        }
        tokio::time::sleep(std::time::Duration::from_millis(25)).await;
    }
