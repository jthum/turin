...
pub async fn run_check(config_path: &Path) -> Result<()> {
    ...
    if let Some(provider_config) = config.providers.get(provider) {
        if let Some(ref env_var) = provider_config.api_key_env {
            if std::env::var(env_var).is_err() {
                println!("\x1b[33m\x1b[1m! Warning:\x1b[0m API key for provider '{}' ({}) is not set in environment.", provider, env_var);
            } else {
                println!("\x1b[32m\x1b[1m✓\x1b[0m API key for provider '{}' is set.", provider);
            }
        }
    } else {
         println!("\x1b[31m\x1b[1m✗\x1b[0m Provider '{}' not found in [providers].", provider);
    }

    // 3. Validate Harness
    let harness_dir = Path::new(&config.harness.directory);
    if !harness_dir.exists() {
        println!("\x1b[33m\x1b[1m! Warning:\x1b[0m Harness directory '{}' does not exist.", harness_dir.display());
    } else {
    ...
    if db_path.exists() {
        println!("\x1b[32m\x1b[1m✓\x1b[0m State database found at '{}'.", db_path.display());
    } else {
        println!("\x1b[34mℹ\x1b[0m State database will be created at '{}' on first run.", db_path.display());
    }
