...
use crate::harness::stdlib::governance_support::current_agent_id;
...
pub(super) fn schedule_create_params(
    lua: &Lua,
...
pub(super) fn schedule_update_params(lua: &Lua, opts: Table) -> LuaResult<ScheduleUpdateParams> {
    let encoded = object_refs::encode_lua_payload(lua, Value::Table(opts))?;
