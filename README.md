...
- **Durable worklists** with hierarchical claimable items, prompt/action payloads, stale-claim recovery, store/scope routing, and daemon query APIs for external inspection (`runtime.worklist.*`, `worklist(...)`, `worklist.list/get/items`, `workitem.get`)
