...
- Worklist-backed `list` nodes render as compact terminal tables with
  terminal-local row-position, filter/sort/limit metadata, and
  action-available cues. When selection moves beyond the first visible page,
  the renderer windows rows around the selected item instead of making
  selection disappear off-screen, and shows the current visible row range plus
  selected row number. Other list sources remain visible with metadata and an
  explicit unsupported-adapter message until the client has a loader for that
  source.
