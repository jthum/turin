...
`.workspace/perf-reports/footprint-1781859846-1957872.md`:

| area | code lines |
| --- | ---: |
| `crates/turin-app` | 4,796 |
| `crates/turin-tui` | 4,127 |
| `crates/turin-ui-core` | 3,705 |
| `crates/turin-web` | 687 |

| static asset | bytes | lines |
| --- | ---: | ---: |
| `crates/turin-web/static/app.css` | 14,192 | 800 |
| `crates/turin-web/static/app.js` | 67,520 | 2,107 |
...
- worklist-backed lists become compact terminal tables with an `action` cue
  ...
  named filter/sort/limit metadata, visible row-range, and selected-row feedback
...
(`.workspace/perf-reports/footprint-1781858846-1940146.md`) reported:

- `86637` Rust code lines under `src` and `crates`
- `4816` code lines in `crates/turin-app`
- `4144` code lines in `crates/turin-tui`
- `3636` code lines in `crates/turin-ui-core`
- `687` code lines in `crates/turin-web`
- `82064` bytes across first-party `turin-web` static assets
