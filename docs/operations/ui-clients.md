...
- Channels: `/` edits the channel filter, `F` clears it, `[` / `]` move across discovered access entries, `a` approves pending rooms, `x` rejects pending rooms, and `v` revokes approved rooms
