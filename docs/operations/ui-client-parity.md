...
| action completed | Latest action result panel near selected harness app; returned UI intents are applied locally. | Latest action result in harness inspector only when it matches the selected app source, including explicit no-payload copy; returned UI intents are applied locally. | Latest action result panel in shell screens and open panes only for the originating app, with action/harness/agent metadata when known; returned UI intents are applied locally. | Backed by `UiUpdate::HarnessActionCompleted` for Rust clients and action API results for web. |
