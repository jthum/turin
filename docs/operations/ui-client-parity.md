...
| `form` | Editable Cast form controls with typed scalar coercion for text, integer, number/float/decimal, boolean, options, multiline fields, and masked password-like fields. | Terminal modal with local drafts, typed scalar coercion for text, integer, number/float/decimal, boolean, options, masked password-like previews, and `Ctrl+J` multiline entry for textarea/markdown fields. | Browser-local drafts with required fields, options, number/float/decimal aliases, integer validation, booleans, textarea, password inputs, and typed scalar coercion. | Rich control fidelity remains client-specific; password-like fields still submit ordinary string params. |
