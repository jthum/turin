...
| `list` | Worklist-backed data table with human-readable field labels, sorted-column markers, filter/sort/limit metadata, row-count and selected-row feedback, app-local row selection, row-level action-availability cues, inline detail with pause/claim/failure context, filtered empty-state copy, and confirmed item-action dispatch when available; unsupported adapters show metadata. | Compact worklist-backed table with human-readable field labels, sorted-column markers, filter/sort/limit metadata, local row selection, row-level action-availability cues, inspector detail with pause/claim/failure context, filtered empty-state copy, and confirmed item-action dispatch when available; unsupported adapters show metadata. | Worklist-backed table through `/api/ui/list`, with human-readable field labels, sorted-column markers, filter/sort/limit metadata, row-count and selected-row feedback, browser-local row selection, keyboard row navigation, row-level action-availability cues, inline detail with pause/claim/failure context, and filtered empty-state copy; unsupported adapters show metadata. | Only named `worklists.<name>` sources have loaders today; app/TUI share stateless visible-node request discovery. |
