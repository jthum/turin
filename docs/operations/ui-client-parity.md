...
| `pane` | Declared but not rendered as a first-class pane yet. | Noted/degraded when shown dynamically. | Exposed through API; shell degrades until a pane model exists. | Needs a client-specific pane model later. |
...
| `list` | Worklist-backed data table; unsupported adapters show metadata. | Compact worklist-backed table with local row selection, inspector detail, and confirmed item-action dispatch when available; unsupported adapters show metadata. | Worklist-backed table through `/api/ui/list`; unsupported adapters show metadata. | Only `worklists.*` sources have loaders today. |
