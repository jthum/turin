...
| `pane` | Rendered as an app-local sheet by default; `modal` and `dialog` presentation hints use a Cast dialog. | Rendered as a terminal-local overlay with pane-local item/action selection when shown. | Exposed through API and rendered as a browser-local overlay when shown. | Pane state remains client-local and unsupported presentation hints degrade by client capability. |
...
| `list` | Worklist-backed data table with human-readable field labels, direction-aware sorted-column markers, named filter/sort/limit metadata, row-count and selected-row feedback, app-local row selection, row-level action-availability cues, inline detail with timeline/worklist/pause/claim/parent/failure context, filtered empty-state copy, and confirmed item-action dispatch when available; unsupported adapters show metadata. | Compact worklist-backed table with human-readable field labels, direction-aware sorted-column markers, named filter/sort/limit metadata, local row selection, row-level action-availability cues, inspector detail with timeline/worklist/pause/claim/parent/failure context, filtered empty-state copy, and confirmed item-action dispatch when available; unsupported adapters show metadata. | Worklist-backed table through `/api/ui/list`, with human-readable field labels, direction-aware sorted-column markers, named filter/sort/limit metadata, row-count and selected-row feedback, browser-local row selection, keyboard row navigation, row-level action-availability cues, inline detail with timeline/worklist/pause/claim/parent/failure context, and filtered empty-state copy; unsupported adapters show metadata. | Only named `worklists.<name>` sources have loaders today; app/TUI share stateless visible-node request discovery. |
...
| `form` | Editable Cast form controls with typed scalar coercion for text, integer, number/float/decimal, boolean, options, textarea/markdown fields, and masked password-like fields. | Terminal modal with local drafts, typed scalar coercion for text, integer, number/float/decimal, boolean, options, masked password-like previews, and `Ctrl+J` multiline entry for textarea/markdown fields. | Browser-local drafts with required fields, options, number/float/decimal aliases, integer validation, booleans, textarea/markdown fields, password inputs, and typed scalar coercion. | Rich control fidelity remains client-specific; password-like fields still submit ordinary string params. |
...
| `chart` | Cast report/chart section with lightweight worklist-backed bar breakdown, grouping hint, counts, and percentage labels; unsupported adapters show metadata. | Lightweight worklist-backed bar breakdown with grouping hint, counts, and percentage labels; unsupported adapters show metadata. | Lightweight worklist-backed bar breakdown with grouping hint, counts, and percentage labels; unsupported adapters show metadata. | App/TUI share stateless grouping semantics; `intent` and `as` remain advisory. |
...
- TUI item selection is local to compact table rows with visible row-range,
  row-position, selected-row, action-available cues, and inspector detail that
  keeps timeline, worklist, pause, claim, parent, failure, metadata, and action context
  visible; selected item actions can be queued for confirmation, page/boundary
  keys work within the focused region, long tables window around the selected
  row instead of hiding it off-screen, and row identity is preserved across
...
- `turin-web` list rows support row-count, selected-row feedback, row-level
  action-availability cues, click, Enter/Space, ArrowUp/ArrowDown, Home, and
  End selection with focus restored after browser-local re-render.
