...
- `runtime.signals.send(topic, payload?, opts) -> 1`
...
- `runtime.signals.send(...)` directs one durable signal to `opts.target_agent`; optional
  `opts.target_session` confines delivery to that exact session and wakes it if cold
