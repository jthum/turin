...
pub const CODE_INDEX_SCHEMA_REVISION: i64 = 2026031004;

pub async fn open_index_connection(index_path: &Path) -> Result<(Database, Connection)> {
...
pub fn encode_vector_blob(vector: &[f32], context: &str) -> Result<Vec<u8>> {
    if vector.is_empty() {
