...
#[derive(Debug, Clone)]
pub struct ExternalRunnerCommand {
    pub program: PathBuf,
    pub args_prefix: Vec<String>,
    pub display: String,
}

impl ExternalRunnerCommand {
...
pub fn external_runner_binary_name(kind: &str) -> String {
    format!("turin-channel-{}", normalize_binary_component(kind))
}

pub fn external_runner_env_var(kind: &str) -> String {
    format!("TURIN_CHANNEL_{}_BIN", normalize_env_component(kind))
}

pub fn resolve_external_runner_command(kind: &str) -> Result<ExternalRunnerCommand> {
...
pub fn describe_external_runner(kind: &str) -> Result<ChannelAdapterManifest> {
    ...
    let output = run_runner_command(&runner, &mut command)?;
...
pub fn validate_external_runner_settings(
    kind: &str,
    settings: &Value,
    ...
    run_runner_command(&runner, &mut command).map(|_| ())
}

pub fn start_external_auth_flow(
    kind: &str,
...
pub fn poll_external_auth_flow(
    kind: &str,
...
pub fn discover_external_runner_kinds() -> Vec<String> {
