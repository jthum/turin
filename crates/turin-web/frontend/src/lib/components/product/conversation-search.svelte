<script lang="ts">
	...
	let { sessionId, onSelect }: { sessionId: string; onSelect: (hit: SearchHit) => void } = $props();
...
<Popover.Root bind:open>
	...
	<Popover.Content align="end" class="w-[min(26rem,calc(100vw-2rem))] p-0">
		<div class="flex items-center gap-2 border-b p-3">
			...
			<Input bind:value={query} class="h-8 border-0 px-0 shadow-none focus-visible:ring-0" placeholder="Search every message" aria-label="Search every message" />
			...
			{:else}
				{#each hits as hit}
					<button class="w-full rounded-lg px-3 py-2.5 text-left hover:bg-muted" onclick={() => select(hit)}>
						...
						<span class="line-clamp-3 text-sm leading-5">{hit.snippet}</span>
					</button>
				{/each}
			{/if}
		</div>
