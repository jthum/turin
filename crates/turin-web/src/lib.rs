...
use turin_types::web_tools::{
    DEFAULT_BRAVE_SEARCH_URL, DEFAULT_BROWSER_ACCEPT, DEFAULT_BROWSER_ACCEPT_ENCODING,
    DEFAULT_BROWSER_ACCEPT_LANGUAGE, DEFAULT_DUCKDUCKGO_SEARCH_URL, DEFAULT_TAVILY_SEARCH_URL,
    WebToolErrorKind, WebToolKind, WebToolOutput, WebToolRequest, WebToolResponse,
    configured_search_providers, fetch_user_agent, search_user_agent, validate_tools_config,
    validate_web_url,
};
use turin_types::{WebFetchToolSettings, WebSearchToolSettings};
use url::Url;
...
pub async fn handle_request(request: WebToolRequest) -> WebToolResponse {
    if let Err(message) = validate_tools_config(&request.tools) {
        return error_response(WebToolErrorKind::Execution, message);
    }

    match request.tool {
        WebToolKind::Fetch => handle_web_fetch(request.params, &request.tools.web_fetch).await,
        WebToolKind::Search => handle_web_search(request.params, &request.tools.web_search).await,
    }
}

fn error_response(kind: WebToolErrorKind, message: impl Into<String>) -> WebToolResponse {
    WebToolResponse::Error {
        kind,
        message: message.into(),
    }
}

fn success_response(content: String, metadata: Value) -> WebToolResponse {
    WebToolResponse::Success {
        output: WebToolOutput { content, metadata },
    }
}

async fn handle_web_fetch(params: Value, settings: &WebFetchToolSettings) -> WebToolResponse {
    let args: WebFetchArgs = match serde_json::from_value(params) {
        Ok(args) => args,
        Err(error) => return error_response(WebToolErrorKind::InvalidParams, error.to_string()),
    };

    let url = match validate_web_url(&args.url) {
        Ok(url) => url,
        Err(error) => return error_response(WebToolErrorKind::InvalidParams, error),
    };

    let client = match build_http_client(args.timeout_secs) {
        Ok(client) => client,
        Err(error) => return error_response(WebToolErrorKind::Execution, error),
    };

    let request = match apply_fetch_headers(client.get(url.clone()), settings) {
        Ok(request) => request,
        Err(error) => return error_response(WebToolErrorKind::Execution, error),
    };
    ...
    let response = match request.send().await {
        ...
        Err(error) => {
            return error_response(
                WebToolErrorKind::Execution,
                format!("web_fetch request failed: {error}"),
            );
        }
    };
    ...
    let title = if content_type.contains("html") {
        extract_html_title(&Html::parse_document(&raw_body))
    } else {
        None
    };
    ...
    success_response(
        ...
        serde_json::json!({
            ...
            "title": title,
            "bytes": body.len(),
            "user_agent": fetch_user_agent(settings),
        }),
    )
}

async fn handle_web_search(params: Value, settings: &WebSearchToolSettings) -> WebToolResponse {
    let args: WebSearchArgs = match serde_json::from_value(params) {
        Ok(args) => args,
        Err(error) => return error_response(WebToolErrorKind::InvalidParams, error.to_string()),
    };
    ...
    if query.is_empty() {
        return error_response(
            WebToolErrorKind::InvalidParams,
    ...
    let providers = match configured_search_providers(settings) {
    ...
    for provider in providers.iter().copied() {
        let result = match provider {
            turin_types::web_tools::WebSearchProvider::Brave => {
                search_brave(&client, settings, query, limit).await
            }
            turin_types::web_tools::WebSearchProvider::Tavily => {
                search_tavily(&client, settings, query, limit).await
            }
            turin_types::web_tools::WebSearchProvider::Searxng => {
                search_searxng(&client, settings, query, limit).await
            }
            turin_types::web_tools::WebSearchProvider::DuckDuckGoHtml => {
                search_duckduckgo_html(&client, settings, query, limit).await
            }
        };
    ...
    error_response(
        WebToolErrorKind::Execution,
        format!("web_search failed across providers: {}", errors.join(" | ")),
    )
}
...
fn header_value(value: &str, label: &str) -> Result<HeaderValue, String> {
    HeaderValue::from_str(value).map_err(|e| format!("Invalid header value for {label}: {e}"))
}
...
fn apply_api_headers(
    ...
    Ok(request.header(ACCEPT, HeaderValue::from_static("application/json")))
}
...
async fn read_json_response<T: for<'de> Deserialize<'de>>(
    ...
    let body = response
        .text()
        .await
        .map_err(|e| format!("{label} body read failed: {e}"))?;
    ...
    serde_json::from_str(&body).map_err(|e| format!("{label} response decode failed: {e}"))
}

fn load_required_api_key(env_name: &str, provider: &str) -> Result<String, String> {
    env::var(env_name)
        .map_err(|_| format!("{provider} search requires environment variable '{env_name}'"))
}
...
async fn search_duckduckgo_html(
    ...
    let response = request
        .send()
        .await
        .map_err(|e| format!("duckduckgo_html search request failed: {e}"))?;
    let html = response
        .text()
        .await
        .map_err(|e| format!("duckduckgo_html search body read failed: {e}"))?;
...
async fn search_brave(
    ...
    let env_name = settings
        ...
        .api_key_env
        .as_deref()
        .ok_or_else(|| "Brave search is not configured".to_string())?;
...
async fn search_tavily(
    ...
    let env_name = settings
        ...
        .api_key_env
        .as_deref()
        .ok_or_else(|| "Tavily search is not configured".to_string())?;
...
async fn search_searxng(
    ...
    let base_url = settings
        ...
        .base_url
        .as_deref()
        .ok_or_else(|| "SearXNG search is not configured".to_string())?;
    ...
    let response = request
        .send()
        .await
        .map_err(|e| format!("searxng search request failed: {e}"))?;
