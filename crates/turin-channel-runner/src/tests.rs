...
#[test]
fn channel_settings_json_requires_object() {
    assert!(parse_channel_settings_json(r#"{"task_timeout_ms":100}"#).is_ok());
    let err = parse_channel_settings_json("[]").unwrap_err();
    assert!(
        err.to_string()
            .contains("Channel settings must be a JSON object")
    );
}
